...
for i in tqdm(data):
        book_id.append(i[0])
        book_name.append(i[2].lower())
        genre.append(i[5])
        summary.append(i[6])
        author.append(i[3].lower())

data = pd.DataFrame({'book_id': book_id, 'title': book_name,  'Genre': genre, 'Summary': summary, 'Author': author})
...
def findAuthor(title):
    ...
    rows = data[data['title'] == title.lower()]
...
def findBooksGenre(genrelist,title):
    ...
    for i in genrelist:
        rows=(data[data['Genre'].str.contains(i,na=False)])
    l=[]
    ...
    mainrow=(data[data['title'].str.contains(title.lower(),na=False)])
...
def findBooksAuthor(author,title):
    ...
    rows=data[data['Author']==str(author)]
